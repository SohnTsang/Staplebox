...
class MoveEntitiesSerializer(serializers.Serializer):
    entity_ids = serializers.ListField(child=serializers.UUIDField(), write_only=True)
    entity_types = serializers.ListField(child=serializers.CharField(), write_only=True)
    target_folder_id = serializers.UUIDField(write_only=True)

    def validate(self, data):
        target_folder = Folder.objects.filter(uuid=data['target_folder_id']).first()
        ...
        for entity_id, entity_type in zip(data['entity_ids'], data['entity_types']):
            if entity_type == 'folder':
                ...
                if Folder.objects.filter(uuid=entity_id, parent=data['target_folder_id']).exists():
    ...
    def _is_descendant_of(self, folder_id, potential_parent_id):
        ...
        """
        current_folder = Folder.objects.get(uuid=folder_id)
        while current_folder.parent:
            if current_folder.parent.uuid == potential_parent_id:
                return True
...
class ProductSerializer(serializers.ModelSerializer):
    class Meta:
        model = Product
        fields = ['uuid', 'product_code', 'product_name', 'product_description', 'product_type', 'created_at', 'updated_at']
        extra_kwargs = {
            'product_name': {
                'error_messages': {
